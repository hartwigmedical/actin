...
class EfficacyEntryFactoryTest {
    ...
    fun `Should convert minimal test extended evidence database`() {
        val result =
            EfficacyEntryFactory.extractCkbExtendedEvidence(CkbExtendedEvidenceTestFactory.createMinimalTestExtendedEvidenceDatabase())
    ...
    fun `Should convert therapies`() {
        ...
        val actual = EfficacyEntryFactory.convertTherapies(
            listOf(
                CkbTherapy(
                    id = 1,
                    therapyName = name,
    ...
    fun `Should convert therapeutic setting to Intent`() {
        val actual = EfficacyEntryFactory.extractTherapeuticSettingFromString("Adjuvant")
    ...
    fun `Should return exception when unknown therapeutic setting is provided`() {
        EfficacyEntryFactory.extractTherapeuticSettingFromString("Unknown therapeutic setting")
    }
    ...
    fun `Should convert variant requirements`() {
        ...
        val actual = EfficacyEntryFactory.convertVariantRequirements(
            listOf(
    ...
    fun `Should convert primary tumor locations when provided in different format`() {
        val actual = EfficacyEntryFactory.convertPrimaryTumorLocation("Colon: 292 (58.5%), Rectum: 207 (41.5%)")
    ...
    fun `Should convert metastatic sites`() {
        val actual = EfficacyEntryFactory.convertMetastaticSites("Liver only: 58 (32%), Lung only: 10 (6%)")
    ...
    fun `Should throw exception if incorrect metastatic sites formatting`() {
        EfficacyEntryFactory.convertMetastaticSites("Liver only: 58-32%, Lung only: 10-6%")
    }

    @Test
    fun `Should convert time of metastases`() {
        val actual = EfficacyEntryFactory.convertTimeOfMetastases("Metachronous")
    ...
    fun `Should throw exception if unknown time of metastases`() {
        EfficacyEntryFactory.convertMetastaticSites("Two months after primary diagnosis")
    }

    @Test
    fun `Should convert region`() {
        val actual = EfficacyEntryFactory.convertRaceOrRegion("North America/Western Europe/Australia: 154, Rest of world: 354")
    ...
    fun `Should throw exception region formatting is incorrect`() {
        EfficacyEntryFactory.convertRaceOrRegion("North America/Western Europe/Australia, Rest of world: 354")
    }

    @Test
    fun `Should convert race`() {
        val actual = EfficacyEntryFactory.convertRaceOrRegion("\nBlack: 2,\nWhite: 26")
    ...
    fun `Should throw exception if race formatting is incorrect`() {
        EfficacyEntryFactory.convertRaceOrRegion("Black, White: 26")
    }

    @Test
    fun `Should convert confidence interval`() {
        val actual = EfficacyEntryFactory.convertConfidenceInterval("4.0 - 6.8")
    ...
    fun `Should throw exception if invalid confidence interval is provided`() {
        EfficacyEntryFactory.convertConfidenceInterval("4.0;6.8")
    }

    @Test
    fun `Should use primary end point value when number is provided`() {
        val monthsEndPointType = EfficacyEntryFactory.convertPrimaryEndPointValue("16.0", "Months")
        assertThat(monthsEndPointType).isEqualTo(16.0)
    }

    @Test(expected = IllegalStateException::class)
    fun `Should throw exception when primary end point value is not a number`() {
        EfficacyEntryFactory.convertPrimaryEndPointValue("invalid input", "Months")
    }

    @Test
    fun `Should set primary end point value to 1 when Y is provided and primary end point type is YN`() {
        val yesEndPointType = EfficacyEntryFactory.convertPrimaryEndPointValue("Y", "Y/N")
    ...
    fun `Should set primary end point value to 0 when N is provided and primary end point type is YN`() {
        val noEndPointType = EfficacyEntryFactory.convertPrimaryEndPointValue("N", "Y/N")
    ...
    fun `Should throw exception when primary end point type is YN and primary end point value is neither Y or N`() {
        EfficacyEntryFactory.convertPrimaryEndPointValue("invalid input", "Y/N")
    }

    @Test
    fun `Should return null primary end point value when primary end point value is NR`() {
        val NREndPointType = EfficacyEntryFactory.convertPrimaryEndPointValue("NR", "Months")
    ...
    fun `Should convert derived metric`() {
        ...
        val actual = EfficacyEntryFactory.convertDerivedMetric(listOf(json))
        val expected = listOf(
            DerivedMetric(
                relativeMetricId = 1,
                value = 16.0,
