...
class HasLimitedSystemicImmuneInflammationIndexTest {
    ...
    fun `Should evaluate to undetermined when no lymphocytes result present`() {
        ...
        EvaluationAssert.assertEvaluation(EvaluationResult.UNDETERMINED, evaluation, "No measurement found for absolute lymphocytes count")
    ...
    fun `Should evaluate to undetermined when lab value date is before min valid date`() {
        ...
        EvaluationAssert.assertEvaluation(
            ...
            evaluation,
            "Most recent measurement too old for absolute lymphocytes count"
        )
        assertThat(evaluation.recoverable).isTrue
    }
    ...
    fun `Should pass when systemic immune inflammation index sufficient`() {
        ...
        EvaluationAssert.assertEvaluation(EvaluationResult.PASS, evaluation, "Systemic immune-inflammation index at most 900.0")
        assertThat(evaluation.recoverable).isTrue
    }
    ...
    fun `Should fail when systemic immune inflammation index insufficient`() {
        ...
        assertThat(evaluation.recoverable).isTrue
        EvaluationAssert.assertEvaluation(EvaluationResult.FAIL, evaluation, "Systemic immune-inflammation index above 900.0")
    }
}