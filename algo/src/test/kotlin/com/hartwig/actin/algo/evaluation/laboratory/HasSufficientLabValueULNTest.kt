...
class HasSufficientLabValueULNTest {
    ...
    fun canEvaluate() {
        ...
        assertEvaluation(EvaluationResult.PASS, function.evaluate(record, LabTestFactory.builder().value(120.0).refLimitUp(200.0).build()))
        assertEvaluation(EvaluationResult.UNDETERMINED, function.evaluate(record, LabTestFactory.builder().value(80.0).build()))
        assertEvaluation(EvaluationResult.FAIL, function.evaluate(record, LabTestFactory.builder().value(50.0).refLimitUp(150.0).build()))
    }
}