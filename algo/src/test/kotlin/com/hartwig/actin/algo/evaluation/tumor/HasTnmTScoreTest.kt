...
class HasTnmTScoreTest {
    ...
    fun `Should be undetermined if the tumor is TNM M`(){
        assertEvaluation(
            EvaluationResult.UNDETERMINED, function("T1", TumorTestFactory.withTumorStage(TumorStage.IV))
        )
    }
    ...
    fun `Should pass if the score matches the tumor stage`(){
        assertEvaluation(
            EvaluationResult.PASS, function("T4", TumorTestFactory.withTumorStage(TumorStage.IIIA))
        )
    }
    ...
    fun `Should fail if the score is not possible with the tumor stage`(){
        assertEvaluation(
            EvaluationResult.FAIL, function("T2A", TumorTestFactory.withTumorStage(TumorStage.IIA))
        )
    }
}
