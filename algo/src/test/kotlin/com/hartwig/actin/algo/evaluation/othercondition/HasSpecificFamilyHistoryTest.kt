...
class HasSpecificFamilyHistoryTest {
    ...
    private val undeterminedConditions =
        UndeterminedFamilyConditions("cardiovascular disease", listOf(IcdConstants.FAMILY_HISTORY_OF_CARDIOVASCULAR_DISEASE_CODE))
    private val passConditions = PassFamilyConditions("sudden death", listOf("familySuddenDeathCode"))
    ...
    fun `Should fail when no conditions belonging to 'pass conditions', 'undetermined conditions', 'other specified', or unspecified family history present`() {
        val condition = OtherConditionTestFactory.priorOtherCondition(icdCode = IcdConstants.HEART_FAILURE_BLOCK)
        assertEvaluation(
    ...
    fun `Should pass when 'pass condition' family history present`() {
        val condition = OtherConditionTestFactory.priorOtherCondition(icdCode = passConditions.icdCodes.first())
    ...
    fun `Should evaluate to undetermined when 'undetermined condition' family history present`() {
        val condition =
            OtherConditionTestFactory.priorOtherCondition(name = "acute myocard infarct", icdCode = undeterminedConditions.icdCodes.first())
        val evaluation = function.evaluate(OtherConditionTestFactory.withPriorOtherCondition(condition))
    ...
    fun `Should evaluate to undetermined when unspecified or 'other specified' family history present`() {
        listOf(
            ...
            assertEvaluation(
                ...
                function.evaluate(
                    OtherConditionTestFactory.withPriorOtherCondition(OtherConditionTestFactory.priorOtherCondition(icdCode = it))
                )
            )
        }
    }
