...
class ExtendedEvidenceEntryFactoryTest {
    ...
    fun `Should convert minimal test extended evidence database`() {
        val result =
            EXTENDED_EVIDENCE_ENTRY_FACTORY.extractCkbExtendedEvidence(CkbExtendedEvidenceTestFactory.createMinimalTestExtendedEvidenceDatabase())
    ...
    fun `Should convert proper test extended evidence database`() {
        val result =
            EXTENDED_EVIDENCE_ENTRY_FACTORY.extractCkbExtendedEvidence(CkbExtendedEvidenceTestFactory.createProperTestExtendedEvidenceDatabase())
        assertThat(result).isNotNull
    }

    @Test
    fun `Should convert therapies`() {
        val actual = ExtendedEvidenceEntryFactory(TREATMENT_DATABASE).convertTherapies(
            listOf(
                CkbTherapy(
                    id = 1,
    ...
    fun `Should convert therapeutic setting to Intent`() {
        val actual = EXTENDED_EVIDENCE_ENTRY_FACTORY.extractTherapeuticSettingFromString("Adjuvant")
    ...
    fun `Should return exception when unknown therapeutic setting is provided`() {
        EXTENDED_EVIDENCE_ENTRY_FACTORY.extractTherapeuticSettingFromString("Unknown therapeutic setting")
    }
    ...
    fun `Should convert variant requirements`() {
        ...
        val actual = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertVariantRequirements(
            listOf(
    ...
    fun `Should use gender count when provided`() {
        val genderProvided = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertGender("10", "15", "25")
        assertThat(genderProvided).isEqualTo(10)
    }

    @Test
    fun `Should derive gender count from other gender and total when count not provided`() {
        val genderNotProvided = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertGender(null, "15", "25")
        assertThat(genderNotProvided).isEqualTo(10)
    }

    @Test
    fun `Should return null gender when neither count is provided`() {
        val bothGendersNotProvided = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertGender(null, null, "25")
        assertThat(bothGendersNotProvided).isEqualTo(null)
    }

    @Test
    fun `Should convert primary tumor locations`() {
        val actual = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertPrimaryTumorLocation("{\"right\": 45, \"left\": 136, \"both or unknown\": 2}")
    ...
    fun `Should convert primary tumor locations when provided in different format`() {
        val actual = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertPrimaryTumorLocation("Colon: 292 (58.5%), Rectum: 207 (41.5%)")
    ...
    fun `Should convert metastatic sites`() {
        val actual = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertMetastaticSites("Liver only: 58 (32%), Lung only: 10 (6%)")
    ...
    fun `Should throw exception if incorrect metastatic sites formatting`() {
        EXTENDED_EVIDENCE_ENTRY_FACTORY.convertMetastaticSites("Liver only: 58-32%, Lung only: 10-6%")
    }

    @Test
    fun `Should convert time of metastases`() {
        val actual = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertTimeOfMetastases("Metachronous")
    ...
    fun `Should throw exception if unknown time of metastases`() {
        EXTENDED_EVIDENCE_ENTRY_FACTORY.convertMetastaticSites("Two months after primary diagnosis")
    }

    @Test
    fun `Should convert region`() {
        val actual = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertRaceOrRegion("North America/Western Europe/Australia: 154, Rest of world: 354")
    ...
    fun `Should throw exception region formatting is incorrect`() {
        EXTENDED_EVIDENCE_ENTRY_FACTORY.convertRaceOrRegion("North America/Western Europe/Australia, Rest of world: 354")
    }

    @Test
    fun `Should convert race`() {
        val actual = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertRaceOrRegion("\nBlack: 2,\nWhite: 26")
    ...
    fun `Should throw exception if race formatting is incorrect`() {
        EXTENDED_EVIDENCE_ENTRY_FACTORY.convertRaceOrRegion("Black, White: 26")
    }

    @Test
    fun `Should convert confidence interval`() {
        val actual = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertConfidenceInterval("4.0 - 6.8")
    ...
    fun `Should throw exception if invalid confidence interval is provided`() {
        EXTENDED_EVIDENCE_ENTRY_FACTORY.convertConfidenceInterval("4.0;6.8")
    }

    @Test
    fun `Should use primary end point value when number is provided`() {
        val monthsEndPointType = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertPrimaryEndPointValue("16.0", "Months")
        assertThat(monthsEndPointType).isEqualTo(16.0)
    }

    @Test(expected = IllegalStateException::class)
    fun `Should throw exception when primary end point value is not a number`() {
        EXTENDED_EVIDENCE_ENTRY_FACTORY.convertPrimaryEndPointValue("invalid input", "Months")
    }

    @Test
    fun `Should set primary end point value to 1 when Y is provided and primary end point type is YN`() {
        val yesEndPointType = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertPrimaryEndPointValue("Y", "Y/N")
    ...
    fun `Should set primary end point value to 0 when N is provided and primary end point type is YN`() {
        val noEndPointType = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertPrimaryEndPointValue("N", "Y/N")
    ...
    fun `Should throw exception when primary end point type is YN and primary end point value is neither Y or N`() {
        EXTENDED_EVIDENCE_ENTRY_FACTORY.convertPrimaryEndPointValue("invalid input", "Y/N")
    }

    @Test
    fun `Should return null primary end point value when primary end point value is NR`() {
        val NREndPointType = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertPrimaryEndPointValue("NR", "Months")
    ...
    fun `Should convert derived metric`() {
        ...
        val actual = EXTENDED_EVIDENCE_ENTRY_FACTORY.convertDerivedMetric(listOf(json))
        val expected = listOf(
            DerivedMetric(
                relativeMetricId = 1,
                value = 16.0,
    ...
    companion object {
        ...
        private val EXTENDED_EVIDENCE_ENTRY_FACTORY = ExtendedEvidenceEntryFactory(TREATMENT_DATABASE)
    }
}