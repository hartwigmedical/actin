...
class LabValueDescendingDateComparatorTest {
    ...
    fun `Should sort lab values on date putting the highest value first if the same measure is measured multiple times on the same day`() {
        ...
        val values = listOf(value1, value2, value3, value4).sortedWith(LabValueDescendingDateComparator(true))
    ...
    fun `Should sort lab values on date putting the lowest value first if the same measure is measured multiple times on the same day`() {
        ...
        val value3 = minimal.copy(date = LocalDate.of(2018, 1, 1), measurement = LabMeasurement.CALCIUM, value = 1.0)
        val value4 = minimal.copy(date = LocalDate.of(2018, 1, 1), measurement = LabMeasurement.CALCIUM, value = 2.0)
        val values = listOf(value1, value2, value3, value4).sortedWith(LabValueDescendingDateComparator(false))

        assertThat(values).containsExactly(value2, value3, value4, value1)
    }
}