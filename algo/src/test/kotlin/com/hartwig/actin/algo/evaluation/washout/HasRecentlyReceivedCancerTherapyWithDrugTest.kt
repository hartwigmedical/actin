...
class HasRecentlyReceivedCancerTherapyWithDrugTest {
    ...
    fun `Should fail no medications`() {
        assertEvaluation(
            ...
            "No recent treatments with name correct in provided treatments"
        )
    }
    ...
    fun `Should pass on medication with recent date`() {
        assertEvaluation(
            ...
            "Recent treatment with medication correct - pay attention to washout period"
        )
    }
    ...
    fun `Should be undetermined with treatment history entry with drug with correct name but without date`() {
        assertEvaluation(
            ...
            "Treatment containing correct in provided treatments with unknown date"
        )
    }
}