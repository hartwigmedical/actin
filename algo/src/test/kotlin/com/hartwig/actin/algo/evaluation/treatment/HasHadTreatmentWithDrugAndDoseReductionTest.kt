...
class HasHadTreatmentWithDrugAndDoseReductionTest {
    ...
    fun `Should fail for empty treatment history`() {
        assertEvaluation(
            ...
            "Patient did not dose reduction receive Drug(name=match, synonyms=[], drugTypes=[], category=TARGETED_THERAPY, displayOverride=null) during treatment"
        )
    }
    ...
    fun `Should be undetermined if matching drugs in treatment history`() {
        ...
        assertEvaluation(
            ...
            function.evaluate(withTreatmentHistory(treatmentHistory)),
            "Undetermined if patient may have had a dose reduction during Drug(name=match, synonyms=[], drugTypes=[], category=TARGETED_THERAPY, displayOverride=null) treatment"
        )
    }
