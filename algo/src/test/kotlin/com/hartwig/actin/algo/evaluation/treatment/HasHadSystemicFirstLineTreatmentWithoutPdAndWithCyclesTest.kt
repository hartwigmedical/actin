...
class HasHadSystemicFirstLineTreatmentWithoutPdAndWithCyclesTest {
    ...
    fun `Should fail for empty treatments`() {
        assertEvaluation(
            ...
            "No Matching as first-line treatment without PD and with at least 3 cycles"
        )
    }
    ...
    fun `Should warn when correct treatment is first line without PD but with insufficient cycles`() {
        assertEvaluation(
            ...
            "Matching as first-line treatment without PD but less than 3 cycles"
        )
    }
    ...
    fun `Should pass when correct treatment is first line without PD and with sufficient cycles`() {
        assertEvaluation(
            ...
            "Matching as first-line treatment without PD and with at least 3 cycles"
        )
    }
    ...
    fun `Should be undetermined first line treatment is unknown trial treatment`() {
        assertEvaluation(
            ...
            "Undetermined if first line trial treatment included Matching"
        )
    }
    ...
    fun `Should be undetermined when correct treatment has unknown date and multiple treatments in history`() {
        assertEvaluation(
            ...
            "Matching but undetermined if first line (dates missing)"
        )
    }
    ...
    fun `Should be undetermined when correct treatment has unknown PD status and other criteria are met`() {
        assertEvaluation(
            ...
            "Matching as first-line treatment but undetermined if without PD"
        )
    }
    ...
    fun `Should be undetermined when correct treatment has unknown number of cycles and other criteria are met`() {
        assertEvaluation(
            ...
            "Matching as first-line treatment but undetermined if with at least 3 cycles"
        )
    }
}