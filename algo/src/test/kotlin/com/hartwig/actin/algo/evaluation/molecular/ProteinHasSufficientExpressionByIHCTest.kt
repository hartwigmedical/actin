...
class ProteinHasSufficientExpressionByIHCTest {
    ...
    fun canEvaluate() {
        ...
        assertEvaluation(EvaluationResult.UNDETERMINED, exact.evaluate(MolecularTestFactory.withIHCTests(priorTests)))

        // Add test with no result
        priorTests.add(ihcTest())
        assertEvaluation(EvaluationResult.FAIL, exact.evaluate(MolecularTestFactory.withIHCTests(priorTests)))
        ...
        assertEvaluation(EvaluationResult.PASS, exact.evaluate(MolecularTestFactory.withIHCTests(priorTests)))

        // Test with unclear result
        assertEvaluation(
            EvaluationResult.UNDETERMINED,
            exact.evaluate(MolecularTestFactory.withIHCTests(ihcTest(scoreText = "Negative")))
        )
    }
