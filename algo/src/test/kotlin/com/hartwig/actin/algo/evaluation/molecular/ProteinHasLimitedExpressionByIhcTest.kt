...
class ProteinHasLimitedExpressionByIhcTest {
    ...
    fun `Should warn when only score text is provided and exact value is unclear`() {
        ...
        assertMolecularEvaluation(
            ...
            function.evaluate(MolecularTestFactory.withIhcTests(test)),
            "Undetermined if protein 1 expression is at most 2 by IHC"
        )
    }
    ...
    fun `Should fail when only correct IHC test in record has no value`() {
        assertMolecularEvaluation(
            ...
            "protein 1 expression not at most 2 by IHC"
        )
    }
    ...
    fun `Should pass when ihc test below requested value`() {
        ...
        assertMolecularEvaluation(EvaluationResult.PASS, function.evaluate(record), "protein 1 has expression of at most 2 by IHC")
    }
    ...
    fun `Should fail when only lower bound is set and above requested value`() {
        ...
        assertMolecularEvaluation(EvaluationResult.FAIL, function.evaluate(record), "protein 1 expression not at most 2 by IHC")
    }
