...
class IsEligibleForFirstLinePalliativeChemotherapyTest {
    ...
    fun `Should be undetermined when patient has metastatic cancer and previous palliative targeted therapy`() {
        ...
        assertThat(result.undeterminedMessages).containsExactly("Patient had palliative targeted therapy (hence may not be considered eligible for first line palliative chemotherapy)")
    }
    ...
    fun `Should be undetermined when patient has metastatic cancer and no previous palliative therapy`() {
        ...
        assertEvaluation(EvaluationResult.UNDETERMINED, result)
        assertThat(result.undeterminedMessages).containsExactly("Undetermined if considered eligible for first line palliative chemotherapy")
    }
