...
class LabEvaluationTest {
    ...
    fun `Should warn if value is under fold of minimum LLN but inside margin of error`() {
        ...
        assertThat(LabEvaluation.evaluateVersusMinLLN(labValue, 2.0)).isEqualTo(EvaluationResult.WARN)
    }
    ...
    fun `Should pass if value is above fold of minimum ULN`() {
        ...
        assertThat(LabEvaluation.evaluateVersusMinULN(labValue, 1.0)).isEqualTo(EvaluationResult.PASS)
    }

    @Test
    fun `Should fail if value is under fold of minimum ULN and outside margin of error`() {
        val labValue = LabTestFactory.create(value = 55.0, refLimitUp = 50.0)
        assertThat(LabEvaluation.evaluateVersusMinULN(labValue.copy(value = 40.0), 1.0)).isEqualTo(EvaluationResult.FAIL)
    }
    ...
    fun `Should warn if value is under fold of minimum ULN but inside margin of error`() {
        ...
        assertThat(LabEvaluation.evaluateVersusMinULN(labValue, 1.0)).isEqualTo(EvaluationResult.WARN)
    }
    ...
    fun `Should warn if value is above fold of maximum ULN but inside margin of error`() {
        ...
        assertThat(LabEvaluation.evaluateVersusMaxULN(labValue, 2.0)).isEqualTo(EvaluationResult.WARN)
    }
