...
class HasHadPriorConditionWithIcdTest {
    ...
    fun `Should pass for prior condition with direct ICD code match`() {
        val conditions = listOf(OtherConditionTestFactory.priorOtherCondition(icdMainCode = "conditionParentCode"))
    ...
    fun `Should fail for prior condition with non-matching icd code`() {
        val conditions = listOf(OtherConditionTestFactory.priorOtherCondition(icdMainCode = "wrongCode"))
