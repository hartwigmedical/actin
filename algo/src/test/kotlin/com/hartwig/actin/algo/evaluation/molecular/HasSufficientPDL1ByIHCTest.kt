...
class HasSufficientPDL1ByIHCTest {
    ...
    fun `Should fail when no test contains result`() {
        assertEvaluation(EvaluationResult.FAIL, function.evaluate(MolecularTestFactory.withPriorTests(listOf(IHCMolecularTest(pdl1Test)))))
    }
    ...
    fun `Should fail when test value is too low`() {
        assertEvaluation(
            ...
            function.evaluate(MolecularTestFactory.withPriorTests(listOf(IHCMolecularTest(pdl1Test.copy(scoreValue = 1.0)))))
        )
    }
    ...
    fun `Should pass when test value is over limit`() {
        assertEvaluation(
            ...
            function.evaluate(MolecularTestFactory.withPriorTests(listOf(IHCMolecularTest(pdl1Test.copy(scoreValue = 3.0)))))
        )
    }
