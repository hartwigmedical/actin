...
class HasRecentlyReceivedCYPXInducingMedicationTest {
    ...
    fun shouldPassWhenCYPInducingMedication() {
        val function = CurrentlyGetsCYPXInducingMedication(MedicationTestFactory.alwaysActive(), "9A9")
        val medications: MutableList<Medication> = mutableListOf()
        medications.add(TestMedicationFactory.builder().addCypInteractions(ImmutableCypInteraction.builder().cyp("9A9").type(CypInteraction.Type.INDUCER).strength(CypInteraction.Strength.MODERATE).build()).build())
