...
class HasAnyHLATypeTest {
    ...
    fun `Should pass if correct HLA allele present`() {
        evaluateFunctions(
            ...
            "HLA type hla-a*02:01 in provided molecular data (allele present without somatic variants in tumor)"
        )
    }
    ...
    fun `Should fail on HLA group match if matchOnHlaGroup is false`() {
        assertMolecularEvaluation(
            ...
            "HLA type a*02:01 or a*02:07 not in provided molecular data"
        )
    }
    ...
    fun `Should warn if correct HLA allele present but record does not have sufficient quality`() {
        ...
        evaluateFunctions(
            ...
            record,
            "Required HLA type hla-a*02:01 - however undetermined whether allele is present in tumor"
        )
    ...
    fun `Should warn if correct HLA allele present but tumor copy number is less than 0,5`() {
        evaluateFunctions(
            ...
            "Required HLA type hla-a*02:01 but allele has low copy number in tumor"
        )
    }
    ...
    fun `Should warn if correct HLA allele present but also somatic mutations present`() {
        evaluateFunctions(
            ...
            "Required HLA type hla-a*02:01 but somatic mutation is present in this allele in tumor"
        )
    }
    ...
    fun `Should pass if correct HLA allele present but tumor copy number or somatic mutation status or both are missing`() {
        evaluateFunctions(
            ...
            "HLA type hla-a*02:01 in provided molecular data"
        )
        evaluateFunctions(
            EvaluationResult.PASS,
    ...
    fun `Should fail if correct HLA allele not present`() {
        evaluateFunctions(
            ...
            "HLA type a*02:01 or a*02:07 not in provided molecular data",
            "HLA type a*02 not in provided molecular data"
        )
        evaluateFunctions(
            EvaluationResult.FAIL,
