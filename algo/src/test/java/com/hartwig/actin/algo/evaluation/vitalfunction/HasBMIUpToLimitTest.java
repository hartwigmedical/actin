...
public class HasBMIUpToLimitTest {
    ...
    public void shouldBeUndeterminedWhenNoBodyWeightsProvidedWithExpectedUnit() {
        assertEvaluation(EvaluationResult.UNDETERMINED,
                function.evaluate(VitalFunctionTestFactory.withBodyWeights(Collections.singletonList(ImmutableBodyWeight.builder()
                        .date(now)
                        .value(70)
                        .unit("pound")
                        .build()))));
    ...
    public void shouldPassIfLatestWeightIsLessThanWarnThreshold() {
        Evaluation evaluation = function.evaluate(VitalFunctionTestFactory.withBodyWeights(Arrays.asList(ImmutableBodyWeight.builder()
                .date(now)
                .value(70.57)
                .unit("Kilogram")
                .build(), ImmutableBodyWeight.builder().date(lastYear).value(100.0).unit("Kilogram").build())));
        assertEvaluation(EvaluationResult.PASS, evaluation);
        assertTrue(evaluation.passSpecificMessages()
    ...
    public void shouldFailIfLatestWeightIsGreaterThanFailThreshold() {
        ...
        assertTrue(evaluation.failSpecificMessages().contains("Patient weight 180.3 kg will exceed BMI limit of 40 for height < 2.12 m"));
    }
    ...
    public void shouldWarnIfLatestWeightIsGreaterThanWarnThreshold() {
        ...
        assertTrue(evaluation.warnSpecificMessages().contains("Patient weight 101.0 kg will exceed BMI limit of 40 for height < 1.59 m"));
    }
}