...
public class HasToxicityWithGradeTest {
    ...
    public void canEvaluateGradeOnly() {
        ...
        assertEvaluation(EvaluationResult.FAIL, function.evaluate(withToxicities(toxicities)));

        toxicities.add(builder().grade(1).build());
        assertEvaluation(EvaluationResult.FAIL, function.evaluate(withToxicities(toxicities)));

        toxicities.add(builder().grade(2).build());
        assertEvaluation(EvaluationResult.PASS, function.evaluate(withToxicities(toxicities)));
    }
    ...
    public void canEvaluateQuestionnaireToxicity() {
        ...
        assertEvaluation(EvaluationResult.PASS, match.evaluate(withToxicities(toxicities)));
        ...
        assertEvaluation(EvaluationResult.UNDETERMINED, noMatch.evaluate(withToxicities(toxicities)));

        toxicities.add(builder().grade(HasToxicityWithGrade.DEFAULT_QUESTIONNAIRE_GRADE + 2).build());
        assertEvaluation(EvaluationResult.PASS, noMatch.evaluate(withToxicities(toxicities)));
    }
