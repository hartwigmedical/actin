...
class HasHadLimitedSystemicTreatmentsExcludingCurativeNeoadjuvantOrAdjuvantStartedSomeMonthsBeforeNextLine(
    ...
    override fun evaluate(record: PatientRecord): Evaluation {
        ...
        return when {
            maxPotentialCount <= maxSystemicTreatments ->
                EvaluationFactory.pass("Received at most $maxSystemicTreatments systemic treatments")

            minCertainCount <= maxSystemicTreatments -> {
                ...
                EvaluationFactory.undetermined(
                    "Undetermined if received at most $maxSystemicTreatments systemic treatments since it is unclear if (neo)adjuvant " +
                            "treatment(s) resulted in PD within $maxMonthsBeforeNextLine months after stopping$undeterminedMessageEnding"
                )
            }

            else -> EvaluationFactory.fail("Has not received at most $maxSystemicTreatments systemic treatments")
        }
    }
}