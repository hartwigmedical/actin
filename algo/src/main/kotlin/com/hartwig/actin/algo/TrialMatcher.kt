...
class TrialMatcher(private val evaluationFunctionFactory: EvaluationFunctionFactory) {
    ...
    private fun warnIfPreviouslyParticipatedInSameTrial(trial: Trial): Eligibility {
        val hasParticipatedInSameTrial = EligibilityFunction(
            EligibilityRule.HAS_PREVIOUSLY_PARTICIPATED_IN_TRIAL_WITH_ACRONYM_X.name,
            listOf(StringParameter(trial.identification.acronym))
        )
        return Eligibility(
            references = setOf(),
            function = EligibilityFunction(EligibilityRule.WARN_IF.name, listOf(FunctionParameter(hasParticipatedInSameTrial)))
        )
    }
