...
class DrugInteractionsDatabase(private val drugInteractionsPerDrug: Map<String, DrugInteractions>) {

    fun annotateWithCypInteractions(
        medicationName: String
    ): List<DrugInteraction> {
        return drugInteractionsPerDrug[medicationName.lowercase()]?.cypInteractions ?: emptyList()
    }

    fun annotateWithTransporterInteractions(
        medicationName: String
    ): List<DrugInteraction> {
        return drugInteractionsPerDrug[medicationName.lowercase()]?.transporterInteractions ?: emptyList()
    }

    companion object {
        fun create(tsv: String): DrugInteractionsDatabase {
            ...
            val interactions = reader.readValues<DrugInteractionEntry>(File(tsv)).readAll()
                ...
                .mapValues { (_, entries) ->
                    getDrugInteractions(entries)
                }
            return DrugInteractionsDatabase(interactions)
        }

        private fun getDrugInteractions(entries: List<DrugInteractionEntry>): DrugInteractions {
            ...
            return DrugInteractions(cypInteractions = cypInteractions, transporterInteractions = transporterInteractions)
        }
