...
internal object QuestionnaireReader {
    ...
    fun read(entryText: String, validKeys: List<String>, sectionHeaders: List<String>): Array<String> {
        return merge(clean(entryText, validKeys).split("\\n").dropLastWhile { it.isEmpty() }.toTypedArray(), validKeys, sectionHeaders)
    }
    ...
    private fun merge(lines: Array<String>, validKeys: List<String>, sectionHeaders: List<String>): Array<String> {
        ...
        for (i in lines.indices) {
            ...
            if (lines[i].trim { it <= ' ' }.isEmpty() || i == lines.size - 1 || isField(
                ...
                || sectionHeaders.any { lines[i + 1].contains(it) }
