...
class QuestionnaireExtraction(private val questionnaireRawEntryMapper: QuestionnaireRawEntryMapper) {
    fun extract(entry: QuestionnaireEntry?): Questionnaire? {
        ...
        return ImmutableQuestionnaire.builder()
            ...
            .tumorType(
                if (QuestionnaireVersion.Companion.version(entry) == QuestionnaireVersion.V0_1) "Unknown" else value(
                    lines,
    ...
    companion object {
        ...
        const val VALUE_LIST_SEPARATOR_2 = ";"
        ...
        private fun lesionData(lines: Array<String?>, keyString: String): LesionData {
            ...
            return if (extractedValues == null) LesionData(null, null) else LesionData.Companion.fromString(
                extractedValues[0],
                extractedValues[1]
            )
        }
        ...
        private fun value(lines: Array<String?>, key: String?, isOptional: Boolean = false): String? {
            ...
            return if (extracted.isEmpty()) null else extracted
        }

        private fun values(lines: Array<String?>, key: String?, lineOffset: Int): List<String>? {
            val lineIndex = lookup(lines, key, false)
            if (lineIndex != null) {
                val extractedValues = Arrays.stream(lines, lineIndex, lineIndex + lineOffset + 1)
