...
internal object TreatmentDB {
    ...
    private val IS_COLORECTAL_CANCER: EligibilityFunction = eligibilityFunction(EligibilityRule.HAS_PRIMARY_TUMOR_LOCATION_BELONGING_TO_DOID_TERM_X, "colorectal cancer")
    private val IS_YOUNG_AND_FIT: EligibilityFunction = eligibilityFunction(EligibilityRule.AND,
    ...
    fun loadTreatments(): List<Treatment> {
        return listOf(combinableChemotherapies(),
                ...
                antiEGFRTherapies().flatMap { therapy: Treatment ->
                    combinableChemotherapies().map { chemo: Treatment ->
                        addComponentsToTreatment(therapy,
                                chemo.name,
                                chemo.components,
                ...
                otherTreatments()).flatten()
    }

    private fun combinableChemotherapies(): List<Treatment> {
        return listOf(createChemotherapy("5-FU", setOf(TreatmentComponent.FLUOROURACIL), SCORE_MONOTHERAPY),
                ...
                createChemotherapy(TREATMENT_FOLFIRI,
                        ...
                        SCORE_MULTITHERAPY,
                        false, setOf(1, 2),
                        setOf(eligibleIfTreatmentNotInHistory(TREATMENT_CAPOX), eligibleIfTreatmentNotInHistory(TREATMENT_FOLFOX),
                                IS_YOUNG_AND_FIT)),
    ...
    private fun createChemotherapy(name: String, components: Set<TreatmentComponent>, score: Int, isOptional: Boolean = false,
                                   lines: Set<Int> = setOf(1, 2), extraFunctions: Set<EligibilityFunction> = emptySet()): Treatment {
        return Treatment(
            name = name,
    ...
    private fun antiEGFRTherapies(): List<Treatment> {
        return listOf(createAntiEGFRTherapy(TREATMENT_CETUXIMAB, TreatmentComponent.CETUXIMAB),
    ...
    private fun createAntiEGFRTherapy(name: String, component: TreatmentComponent): Treatment {
        return Treatment(
            ...
            lines = setOf(2, 3),
            eligibilityFunctions = setOf(IS_COLORECTAL_CANCER,
    ...
    private fun otherTreatments(): List<Treatment> {
        return listOf(createChemotherapy(TREATMENT_LONSURF,
    ...
    private fun addComponentsToTreatment(treatment: Treatment, name: String, components: Set<TreatmentComponent>,
