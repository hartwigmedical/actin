...
class RecommendationEngineTest {
    ...
    fun shouldNotRecommendTheSameChemotherapyAfterStopReasonPD() {
        CHEMO_TREATMENT_NAME_STREAM.forEach { treatmentName: String ->
            val patientRecord: PatientRecord = patientWithTreatment(ImmutablePriorTumorTreatment.builder()
                    .name(treatmentName)
                    .isSystemic(true)
    ...
    fun shouldRecommendAntiEGFRTherapyForPatientsMatchingMolecularCriteria() {
        ...
        assertAntiEGFRTreatmentCount(getTreatmentResultsForPatient(patientRecordWithHistoryAndMolecular(firstLineChemotherapies, emptyList(),
    ...
    fun shouldNotRecommendAntiEGFRTherapyForPatientsMatchingMolecularCriteriaButWithRightSidedTumor() {
        val firstLineChemotherapies = listOf(TreatmentDB.TREATMENT_CAPOX)
        assertAntiEGFRTreatmentCount(getTreatmentResultsForPatient(patientRecordWithHistoryAndMolecular(firstLineChemotherapies, emptyList(),
                MINIMAL_PATIENT_RECORD.molecular(), "Ascending colon")), 0)
    }
    ...
    fun shouldRecommendPembrolizumabForMSI() {
        ...
        val molecularRecord: MolecularRecord = ImmutableMolecularRecord.builder()
                ...
                .build()
        Assert.assertTrue(getTreatmentResultsForPatient(patientRecordWithHistoryAndMolecular(emptyList(), emptyList(),
                molecularRecord)).any { treatment: Treatment -> treatment.name == TreatmentDB.TREATMENT_PEMBROLIZUMAB })
    }
    ...
    fun shouldRecommendCetuximabAndEncorafenibForBRAFV600E() {
        ...
        Assert.assertTrue(getTreatmentResultsForPatient(patientRecordWithHistoryAndMolecular(firstLineChemotherapies, emptyList(),
                TestMolecularFactory.createProperTestMolecularRecord())).any { it.name == "Cetuximab + Encorafenib" })
    }

    @Test
    fun shouldRecommendLonsurfAfterChemoAndTargetedTherapy() {
        val record = patientRecordWithHistoryAndMolecular(listOf(TreatmentDB.TREATMENT_CAPOX), listOf(TreatmentDB.TREATMENT_PANITUMUMAB),
    ...
    fun shouldNotRecommendLonsurfAfterTrifluridine() {
        val record = patientRecordWithHistoryAndMolecular(listOf(TreatmentDB.TREATMENT_CAPOX, "trifluridine"),
    ...
    companion object {
        ...
        private val CHEMO_TREATMENT_NAME_STREAM = listOf("5-FU",
                "Capecitabine",
                "Irinotecan",
                "Oxaliplatin",
        ...
        private fun getTreatmentResultsForPatient(patientRecord: PatientRecord): List<Treatment> {
            val doidModel: DoidModel = TestDoidModelFactory.createWithOneDoidAndTerm(DoidConstants.COLORECTAL_CANCER_DOID, "colorectal cancer")
        ...
        private fun assertMultiChemotherapyNotRecommended(patientRecord: PatientRecord) {
            val chemotherapyComponents = java.util.Set.of(TreatmentComponent.FLUOROURACIL,
        ...
        private fun patientRecordWithTumorDoids(tumorDoid: String): PatientRecord {
            val tumorDetails: TumorDetails = ImmutableTumorDetails.builder().addDoids(DoidConstants.COLORECTAL_CANCER_DOID, tumorDoid).build()
            val clinicalRecord: ClinicalRecord = ImmutableClinicalRecord.builder().from(MINIMAL_PATIENT_RECORD.clinical()).tumor(tumorDetails).build()
        ...
        private fun patientRecordWithHistoryAndMolecular(pastChemotherapyNames: List<String>,
                                                         pastTargetedTherapyNames: List<String>, molecularRecord: MolecularRecord, tumorSubLocation: String? = null): PatientRecord {
        ...
        private fun priorTreatmentsFromNames(names: List<String>, category: TreatmentCategory): List<ImmutablePriorTumorTreatment> {
            return names.map { treatmentName: String -> ImmutablePriorTumorTreatment.builder()
                        .name(treatmentName)
                        .isSystemic(true)
