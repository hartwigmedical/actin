...
class HasHadSpecificTreatmentSinceDateTest {
    ...
    fun shouldFailWhenMatchingTreatmentIsOlderByMonth() {
        ...
        val priorTumorTreatments: List<PriorTumorTreatment> = listOf(nonMatchingTreatment, matchingTreatment(olderDate.year, olderDate.monthValue))
        assertEvaluation(EvaluationResult.FAIL, function.evaluate(TreatmentTestFactory.withPriorTumorTreatments(priorTumorTreatments)))
    }

    @Test
    fun shouldPassWhenPriorTreatmentsIncludeMatchingTreatmentWithinRange() {
        val priorTumorTreatments: List<PriorTumorTreatment> = listOf(nonMatchingTreatment, olderTreatment, matchingTreatment(recentDate.year, recentDate.monthValue))
    ...
    fun shouldFailWhenPriorTreatmentHasUnknownStopDateButOlderStartDate() {
        ...
        val priorTumorTreatments: List<PriorTumorTreatment> = listOf(nonMatchingTreatment,
                olderTreatment,
    ...
    companion object {
        ...
        private val nonMatchingTreatment: PriorTumorTreatment = TreatmentTestFactory.builder()
                ...
                .build()
        private val olderTreatment: PriorTumorTreatment = matchingTreatment(LocalDate.now().minusYears(YEARS_TO_SUBTRACT.toLong()).year, null)

        private fun matchingTreatment(stopYear: Int?, stopMonth: Int?, startYear: Int? = null, startMonth: Int? = null): PriorTumorTreatment {
