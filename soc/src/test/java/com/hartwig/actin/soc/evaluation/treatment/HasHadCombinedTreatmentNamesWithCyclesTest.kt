...
class HasHadCombinedTreatmentNamesWithCyclesTest {
    ...
    fun shouldPassWhenAllQueryTreatmentNamesHaveAtLeastOneMatchWithRequiredCycles() {
        val treatmentHistory = listOf(MATCHING_PRIOR_TREATMENT,
    ...
    fun shouldReturnUndeterminedWhenAnyQueryTreatmentNameHasAtLeastOneMatchWithNullCyclesAndNoneWithRequiredCycles() {
        assertEvaluation(EvaluationResult.UNDETERMINED, function.evaluate(patientRecordWithTreatmentHistory(listOf(MATCHING_PRIOR_TREATMENT,
    ...
    fun shouldFailWhenAnyQueryTreatmentNameHasAllMatchesWithKnownCycleCountOutsideRange() {
        assertEvaluation(EvaluationResult.FAIL, function.evaluate(patientRecordWithTreatmentHistory(listOf(MATCHING_PRIOR_TREATMENT,
    ...
    companion object {
        ...
        private fun patientRecordWithTreatmentHistory(priorTumorTreatments: List<PriorTumorTreatment>): PatientRecord {
            ...
            val clinicalRecord: ClinicalRecord = ImmutableClinicalRecord.copyOf(minimal.clinical()).withPriorTumorTreatments(priorTumorTreatments)
