...
class ReportLabelsTest {
    ...
    fun `Should load labels from resource file`() {
        ...
        assertThat(labels.reportTitle()).isEqualTo("ACTIN Report (research use only)")
    }

    @Test
    fun `Should resolve apostrophe escaping`() {
        val labels = ReportLabels.load(ReportIntendedUse.RESEARCH_USE_ONLY)
        assertThat(labels.trialFootnoteChildrensHospital("2 trials", "See other results."))
    ...
    fun `Should substitute parameters`() {
        val labels = ReportLabels.load(ReportIntendedUse.RESEARCH_USE_ONLY)
        assertThat(labels.footerCtgovDisclaimer("2024-01-01"))
    ...
    fun `Should support non-ASCII characters`() {
        ...
        assertThat(ReportLabels(props).miscNotAvailable()).isEqualTo("Niet beschikbäar")
    }

    @Test
    fun `Should throw exception on missing key`() {
        assertThatThrownBy { ReportLabels(Properties()).reportTitle() }
