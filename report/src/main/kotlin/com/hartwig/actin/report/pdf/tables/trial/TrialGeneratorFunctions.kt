...
object TrialGeneratorFunctions {
    ...
    fun addTrialsToTable(
        ...
        externalTrials.forEach { trial ->
            val phaseText = trial.phase?.takeIf { it != TrialPhase.COMPASSIONATE_USE }?.let { "\n (${it.display()})" } ?: ""
            val trialLabelText = (trial.title.takeIf { it.length < 20 } ?: trial.nctId) + phaseText
    ...
    fun contentForTrialCohortList(
        ...
        val commonEvents = findCommonMembersInCohorts(cohortsForTrial) { it.molecularInclusionEvents.map { e -> e.display() }.toSet() }
        ...
        return prefix + cohortsForTrial.map { cohort: InterpretedCohort ->
            ...
            listOfNotNull(
                cohortString,
                concat(
                    cohort.molecularInclusionEvents.map { it.display() }.toSet() - commonEvents,
                    commonEvents.isEmpty() && (!allEventsEmpty || hidePrefix)
                ),
                if (includeSites) TrialLocations.actinTrialLocation(
                    cohort.sources,
                    requestingSource,
