...
and cohort states, and become aware of new or closed trials and cohorts.

Along with the CTC database itself, the `ctc_config_directory` is expected to contain 2 more files:

- `ignore_studies.tsv`: A single-column file with studies that are present in the CTC database but ACTIN explicitly can ignore
...
The CTC database is used to resolve trial and cohorts states as described below, and in addition the following checks are performed on the
CTC database itself as well as the two configuration files:
...
In case cohorts belonging to a single trial have different open states, a warning is raised and it is assumed the trial is closed.
...
- The `ctcCohortIds` needs to be one of the following:
    ...
    - One of the following specific values, in which case it is assumed the cohort is closed without slots available:
        ...
        - `wont_be_mapped_because_not_available`: The cohort will never be mapped by CTC since the cohort is unavailable in Erasmus MC.
...
- If one or more children are configured, ACTIN takes the "most lenient" option. Cohorts are more lenient if they are open and have slots
  ...
  This check results in a warning, and it is assumed the most lenient child is still the most representative of the actual cohort status.
...
- A list of inputs to rules are expected to be separated by semicolons (for
