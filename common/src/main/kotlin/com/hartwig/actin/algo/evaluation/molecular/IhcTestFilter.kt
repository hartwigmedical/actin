...
object IhcTestFilter {
    ...
    fun allPDL1TestsByMeasureToFind(ihcTests: List<IhcTest>, measureToFind: String?, isLungCancer: Boolean? = null): List<IhcTest> {
        val allPDL1Tests = mostRecentAndUnknownDateIhcTests(ihcTests).filter { test -> test.item == PD_L1 }
        return when {
            measureToFind == null -> emptyList()
            measureToFind == "TPS" && isLungCancer == true && allPDL1Tests.all { it.measure == null } -> allPDL1Tests
            else -> allPDL1Tests.filter { measureToFind == it.measure }
        }
    }
