    ...
    companion object {
        ...
        internal fun filterForSources(actionableEvents: ActionableEvents, sourcesToInclude: Set<Knowledgebase?>): ActionableEvents {
            return ImmutableActionableEvents.builder()
                .hotspots(
                    filterActionableForSources<ActionableHotspot>(
                        actionableEvents.hotspots(),
                        sourcesToInclude
                    )
                )
                .codons(
                    filterActionableForSources<ActionableRange>(
                ...
                .genes(
                    filterActionableForSources<ActionableGene>(
                        actionableEvents.genes(),
                        sourcesToInclude
                    )
                )
                .fusions(
                    filterActionableForSources<ActionableFusion>(
                        actionableEvents.fusions(),
                        sourcesToInclude
                    )
                )
                .characteristics(
                    filterActionableForSources<ActionableCharacteristic>(
                        actionableEvents.characteristics(),
                        sourcesToInclude
                    )
                )
                .hla(
                    filterActionableForSources<ActionableHLA>(
        ...
        private fun fromActionableEvents(
            ...
            val variantEvidence: VariantEvidence =
                VariantEvidence.create(actionableEvents)
            val copyNumberEvidence: CopyNumberEvidence =
                CopyNumberEvidence.create(actionableEvents)
            val homozygousDisruptionEvidence: HomozygousDisruptionEvidence =
                HomozygousDisruptionEvidence.create(actionableEvents)
            val breakendEvidence: BreakendEvidence =
                BreakendEvidence.create(actionableEvents)
            val fusionEvidence: FusionEvidence =
                FusionEvidence.create(actionableEvents)
            val virusEvidence: VirusEvidence =
