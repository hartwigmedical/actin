...
class CombinedEvidenceTrialMatcher(private val evidences: List<EfficacyEvidence>, private val trials: List<ActionableTrial>) {
    val logger: Logger = LogManager.getLogger(CombinedEvidenceTrialMatcher::class.java)

    fun match(molecularTest: MolecularTest): MatchesForActionable {
        val evidences = evidences.asSequence()
            .flatMap { evidence ->
                when (val result = match(molecularTest, evidence.molecularCriterium())) {
                    is ActionabilityMatchResult.Success -> result.actionables.map { actionable -> actionable to evidence }
        ...
        val trials = trials.asSequence()
            .flatMap { trial -> trial.anyMolecularCriteria().map { trial to it } }
            .flatMap { trialAndCriterium ->
                when (val result = match(molecularTest, trialAndCriterium.second)) {
                    is ActionabilityMatchResult.Success -> result.actionables.map { actionable -> actionable to trialAndCriterium.first }
                    is ActionabilityMatchResult.Failure -> emptyList()
                }
            }
            .groupBy(keySelector = { it.first }, valueTransform = { it.second })
        ...
        return evidences.entries.associate { entry ->
            ...
            entry.key to ActionabilityMatch(entry.value.toList(), trialMatches.toList())
        }
    }
