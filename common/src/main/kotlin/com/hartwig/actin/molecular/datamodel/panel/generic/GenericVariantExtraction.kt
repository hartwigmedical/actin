...
data class GenericVariantExtraction(
    val gene: String,
    val hgvsCodingImpact: String
) : PanelEvent {
    companion object {
        fun parseVariant(priorMolecularTest: PriorMolecularTest): GenericVariantExtraction {
            return if (priorMolecularTest.item != null && priorMolecularTest.measure != null) {
                GenericVariantExtraction(gene = priorMolecularTest.item, hgvsCodingImpact = priorMolecularTest.measure)
            } else {
