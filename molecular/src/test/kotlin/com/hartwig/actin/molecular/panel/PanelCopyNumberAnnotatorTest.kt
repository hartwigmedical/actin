...
class PanelCopyNumberAnnotatorTest {
    ...
    fun `Should convert sequenced deletion to full del copy number for canonical transcript`() {
        ...
        val annotatedPanel = annotator.annotate(setOf(SequencedDeletion(GENE, CANONICAL_TRANSCRIPT, 0, false)))
    ...
    fun `Should convert sequenced deletion to full del copy number for non-canonical transcript`() {
        ...
        val annotatedPanel = annotator.annotate(setOf(SequencedDeletion(GENE, NON_CANONICAL_TRANSCRIPT, 0, false)))
        val canonicalImpact = TestTranscriptCopyNumberImpactFactory.createTranscriptCopyNumberImpact(CopyNumberType.NONE)
            .copy(transcriptId = CANONICAL_TRANSCRIPT, minCopies = null, maxCopies = null)
        val otherImpacts = setOf(
    ...
    fun `Should convert sequenced deletion to partial del copy number for non-canonical transcript`() {
        ...
        val annotatedPanel = annotator.annotate(setOf(SequencedDeletion(GENE, NON_CANONICAL_TRANSCRIPT, 0, true)))
        val canonicalImpact = TestTranscriptCopyNumberImpactFactory.createTranscriptCopyNumberImpact(CopyNumberType.NONE)
            .copy(transcriptId = CANONICAL_TRANSCRIPT, minCopies = null, maxCopies = null)
        val otherImpacts = setOf(
    ...
    @Test
    fun `Should convert sequenced deletion to partial del copy number for canonical transcript`() {
        setupEnsemblDataCacheForCopyNumber()

        val annotatedPanel = annotator.annotate(setOf(SequencedDeletion(GENE, CANONICAL_TRANSCRIPT, 0, true)))
