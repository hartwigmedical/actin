...
internal class VariantEvidence private constructor(private val actionableHotspots: MutableList<ActionableHotspot?>,
                                                   private val actionableRanges: MutableList<ActionableRange?>, private val applicableActionableGenes: MutableList<ActionableGene?>) : EvidenceMatcher<PurpleVariant?> {
    override fun findMatches(variant: PurpleVariant): MutableList<ActionableEvent?> {
        return Stream.of(hotspotMatches(variant), rangeMatches(variant), geneMatches(variant))
            .flatMap { obj: MutableList<ActionableEvent?>? -> obj.stream() }
            .collect(Collectors.toList())
    }

    private fun hotspotMatches(variant: PurpleVariant): MutableList<ActionableEvent?> {
        return filterMatchingEvents<ActionableHotspot?>(actionableHotspots, variant) { obj: ActionableHotspot?, hotspot: PurpleVariant? -> HotspotMatching.isMatch(hotspot) }
    }

    private fun rangeMatches(variant: PurpleVariant): MutableList<ActionableEvent?> {
        return filterMatchingEvents<ActionableRange?>(actionableRanges, variant) { obj: ActionableRange?, rangeAnnotation: PurpleVariant? -> RangeMatching.isMatch(rangeAnnotation) }
    }

    private fun geneMatches(variant: PurpleVariant): MutableList<ActionableEvent?> {
        return filterMatchingEvents<ActionableGene?>(applicableActionableGenes, variant) { obj: ActionableGene?, geneAnnotation: PurpleVariant? -> GeneMatching.isMatch(geneAnnotation) }
    }

    private fun <T : ActionableEvent?> filterMatchingEvents(events: MutableList<T?>, variant: PurpleVariant,
                                                            predicate: BiPredicate<T?, PurpleVariant?>): MutableList<ActionableEvent?> {
        return if (!variant.reported()) emptyList<ActionableEvent?>() else events.stream().filter { event: T? -> predicate.test(event, variant) }.collect(Collectors.toList())
    }
