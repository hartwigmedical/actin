...
object VariantDedup {
    ...
    private fun include(variant: SmallVariant, variants: Collection<SmallVariant>): Boolean {
    ...
    private fun hasCanonicalPhasedEffect(variant: SmallVariant): Boolean {
        return variant.transcriptImpact().effects().any { PHASED_EFFECTS.contains(it) }
    }

    private fun hasSameEffectWithHigherVCN(variants: Collection<SmallVariant>, variantToMatch: SmallVariant): Boolean {
        ...
        val variantImpactToMatch = variantToMatch.transcriptImpact()
        for (variant in variants) {
            val variantImpact = variant.transcriptImpact()
