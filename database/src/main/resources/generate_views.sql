...
AS (
SELECT * FROM (
SELECT  x.sampleId, IF(isCancerAssociatedVariant,"Mutation (Cancer-associated variant)",IF(isBiallelic,"Mutation (No known cancer-associated variant, biallelic)", "Mutation (No known cancer-associated variant)")) AS type, x.event, concat(round(variantCopyNumber,1),"/",round(totalCopyNumber,1), " copies") AS details, driverLikelihood,
